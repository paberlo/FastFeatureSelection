...
public class RerankingSearch extends ASSearch implements OptionHandler {
	...
	protected boolean m_equalWidth=false;
	...
	public int[] search(ASEvaluation ASEval, Instances data) throws Exception {
		...
		Instances discretized_data=discretize(data);
		...
		while (anySelected) {
			m_blocksSearched++;
			Instances block;
			if(ASEval.getCapabilities().handles(Capability.NUMERIC_ATTRIBUTES)){
				block = projectBlock(data);
			} else block=projectBlock(discretized_data);
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (selectionString.length() != 0){
			String[] searchSpec=Utils.splitOptions(selectionString);
			String searchName=searchSpec[0];
			searchSpec[0]="";
			setSearchAlgorithm(ASSearch.forName(searchName, searchSpec));
		}
	...
	public String[] getOptions() {
		...
		if(m_searchAlgorithm instanceof OptionHandler)
			searchOptions=((OptionHandler)getSearchAlgorithm()).getOptions();
	...
	protected void rerank(Instances data) throws Exception {
		long start = System.currentTimeMillis();
		//if B is very small, first block search might not select any features, so we do
		//not rerank and just expand later the block with next B features.
	 ...
	 */
	public ASSearch getSearchAlgorithm()  {
	 ...
	 */
	public void setSearchAlgorithm(ASSearch search) throws Exception{
		if(!(search instanceof StartSetHandler)){
			throw new Exception("Search algorithm needs to be a StartSetHandler! (for reranking porpuses)");
		}
		m_searchAlgorithm = search;
	}
 ...
 +"Systems. Volume 25 Issue 1. February 2012\".";
	...
	public void setDiscretizeByEqualWidth(boolean b){
		m_equalWidth=b;
	}
	
	protected String getSearchSpec() {
	    
	    ASSearch s = m_searchAlgorithm;
	...
	protected Instances discretize(Instances data) throws Exception{
		Instances d=null;
		...
		if (data.checkForAttributeType(Attribute.NUMERIC)) {
			...
			if(!m_equalWidth){
				...
				myfilter.setInputFormat(data);
				d = Filter.useFilter(data, myfilter);
			}
		
		else{
			weka.filters.unsupervised.attribute.Discretize equalWidth =
					new weka.filters.unsupervised.attribute.Discretize();
			try {
				System.out.println("ATTENTION!!: NUMERIC ATTRIBUTES DISCRETIZED WITH 2 BINS");
				equalWidth.setInputFormat(data);
				equalWidth.setBins(2);
				d = Filter.useFilter(data, equalWidth);
			} catch (Exception e) {
				e.printStackTrace();
				System.exit(0);
			}
